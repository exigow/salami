...
object FastMath {
  ...
  fun nearestPointToLine(lineStart: Vec2, lineEnd: Vec2, point: Vec2): Vec2 {
    val diff = lineEnd - lineStart
    val len = diff.length()
    val line = diff.normalize()
    val v = point - lineStart
    val d = v.x * line.x +  v.y * line.y
    val clamped = FastMath.clamp(d, 0f, len)
    return lineStart + line * clamped
  }
